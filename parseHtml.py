...
class html2csv(HTMLParser.HTMLParser):
    ...
    def end_tr(self):
        ...
        self.inTR = 0            
        if (len(self.CSVrow) > 0)and (self.notFinished):
    ...
    def handle_data(self, data):
        #if self.inTD:
        if data.find("Test Module Information")>-1:
            self.notFinished=False
        if (
            (data.find("SysTst_")>-1)
            and (not(data.find("_SysTst_")>-1))
            or(data.find("fail")>-1) and (not(data.find("failed")>-1))
            or(data.find("pass")>-1) and (not(data.find("passed")>-1))
            ):
                self.CSVrow += data
